...
本文只保留 NepTrainKit 训练集检查的核心数据契约、算法边界、Finding 语义和性能证据。
用户可见的信息架构与交互验收以 `training-set-audit-product-reset-v2.md` 为唯一基线；
P0 默认状态与签核条件见 `training-set-audit-p0-convergence.md`。

- 状态：算法与证据契约基线，不再定义页面信息架构。
- 生效日期：2026-07-12。
...
打开独立的“训练集评估”页面。页面顶层采用连续工作流：

```text
┌ 概览 ──────┬ 数据地图 ──────┬ 复核队列 ──────┬ 目标与模型 ──────┐
│ 直接盘点    │ 渐进展开证据    │ 只放可执行工作项 │ 用户主动设置后比较 │
└────────────┴────────────────┴─────────────────┴──────────────────┘
```

目标未设置和独立模型证据未接入都是合法状态，页面必须明确说明当前无法判断什么，
不能输出隐式数量规则、假分数或红色告警。
