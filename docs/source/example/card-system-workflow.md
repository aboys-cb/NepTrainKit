...
| 2 | 折叠或展开 | 参数设置完成后折叠，便于查看整条流程和拖动排序 |
| 3 | 在线手册 | 查看这张卡片的完整参数、限制和适用场景 |
| 4 | 卡片信息 | 查看版本、维护者和来源信息 |
| 5 | 复制单张卡片 JSON | 复制当前参数，之后用顶部 `Paste JSON` 追加一张相同卡片 |
| 6 | 查看本卡输出 | 运行成功后才可用，只把这张卡片的结果送到 `NEP Dataset Display` |
| 7 | 导出本卡输出 | 保存这一步的中间结构，不等于保存整条流程 |
| 8 | 关闭卡片 | 从工作区移除；不会删除已经导出的文件 |

拖动卡片标题栏可以调整顺序。改完顺序必须重新运行，因为顶层卡片的输出会依次传给下一张已启用卡片。
...
顶部按钮控制整条流程：`Copy JSON` 复制当前全部卡片，`Paste JSON` 把剪贴板中的配置追加到现有流程，`Run` 运行所有已启用卡片，`Stop` 中止正在运行的卡片。标题栏里的 `Copy JSON` 只复制单张卡片，两者不要混用。
...
- 卡片标题栏的查看、导出按钮：只处理这一张卡片的输出。
- 顶部 `View selected outputs`：合并查看所有已勾选且已经产生结果的卡片，包含中间步骤。
- 顶部 `Save` 菜单中的 `Export final workflow output`：只保存最后完成的已启用卡片输出；`Export all available card outputs` 用于保留全部中间结果。

## 保存的是结构，还是流程

这两个概念必须分开：

- `Copy JSON` 或 `Save → Export Card Config` 保存卡片顺序、参数和启停状态，**不包含生成结构**。
- `Paste JSON` 会把剪贴板中的单张卡片、卡片列表或完整流程追加到当前工作区，不会自动清空原卡片。
- `Load → Import Card Config` 用于从 JSON 文件恢复流程。
- 导出卡片输出或最终流程输出，保存的才是结构数据。

第一次把流程调通后，建议保存一份流程 JSON。更换输入结构时先检查元素、原子数和晶胞是否仍满足这些参数，再运行；不要把同一份流程配置当作所有材料都适用的物理方案。
