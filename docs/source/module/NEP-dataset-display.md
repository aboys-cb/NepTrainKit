...
训练集评估还可以按精确组分查看结构数量，并对全部结构分析 FCC、HCP、BCC、L1₂、Laves 相结构和 `spin:R:3` 磁序。完整用法、判据、公式和限制见 [训练集评估](training-set-assessment.md)。
...
| 完整检查当前数据 | 顶部 `Save` → `评估当前数据集` | 查看阻塞项、组分、局域环境、相结构、磁序和复核队列 |
