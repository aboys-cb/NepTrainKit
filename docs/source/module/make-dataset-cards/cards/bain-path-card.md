...
这张卡不是随机扰动卡。它扫描的是晶胞形状自由度：选定 `x`、`y` 或 `z` 作为 c 轴，按 `c/a 范围`（`ca_range`）改变该方向长度；`constant_volume` 会同步缩放另外两条轴让体积保持不变，`scale_volume` 会在此基础上再扫体积，`free_c` 只改选中轴。

## 原理与公式

设扫描因子为 $r=c/a$，选中的 c 轴写在矩阵最后一维。三种模式对应的形变矩阵为

$$
\mathbf D_{\mathrm{等体积}}=
\operatorname{diag}(r^{-1/2},r^{-1/2},r),\qquad
\mathbf D_{\mathrm{自由c轴}}=\operatorname{diag}(1,1,r).
$$

等体积模式满足 $\det\mathbf D_{\mathrm{等体积}}=1$。`形状加体积`模式再给整个晶胞乘
$v^{1/3}$，即 $\mathbf C'=v^{1/3}\mathbf D_{\mathrm{等体积}}\mathbf C$，所以
$V'/V=v$。选择 x 或 y 轴时只需相应置换对角元。`缩放原子坐标`开启时保持分数坐标不变，
原子随晶胞一起变形；关闭时保持 Cartesian 坐标不变。
...
| `scale_volume` | 先做 constant-volume Bain，再按 `体积缩放范围`（`volume_scale_range`）统一缩放体积 | 同时补四方畸变和体积响应 |
| `free_c` | 只缩放选中轴，另外两轴不动 | 模拟外延约束或单轴加载 |

### 体积缩放范围（volume_scale_range）

`tuple[float, float, float]`，默认 `(1.0, 1.0, 1.0)`。格式是 `[起点, 终点, 步长]`，表示输出体积相对原始体积的比例。

这个参数只在 `mode = "scale_volume"` 时生效。比如 `[0.95, 1.05, 0.05]` 会为每个 `c/a` 生成 0.95、1.00、1.05 三个体积点；如果 `c/a 范围`（`ca_range`）有 5 个点，总输出就是 15 个结构。范围别一上来开太大，体积和形状同时扫很容易把训练集放大。

生效条件：`模式`（`mode`）必须是 `scale_volume`。
...
适合怀疑模型同时缺体积响应和四方畸变响应时使用，输出数量是 `c/a 范围`（`ca_range`）点数乘以体积点数。
...
**体积没有保持不变。** 检查 `模式`（`mode`）是否为 `constant_volume`。`free_c` 会改变体积，`scale_volume` 会按 `体积缩放范围`（`volume_scale_range`）继续改体积。

**结构看起来被拉得太夸张。** 缩小 `c/a 范围`（`ca_range`），先用 `0.95-1.05` 验证模型响应，再决定是否覆盖更宽相变路径。
