...
列表。组内的子卡片列表，由界面拖入操作自动维护。每项是一个完整的卡片配置 dict（含 class、check_state、params 等字段）。子卡片共享组输入，互不串联。

### `filter_card`

dict 或 null，默认 `null`。在组内末尾安放一张过滤卡（如 `FPS Filter`），对汇总结果做选筛。为 null 表示不启用组内过滤。

> 组内过滤只作用于组输出，不作为下游其他卡片的输入源。如果需要在组外继续处理，请把过滤卡串在组后。
