...
| 选项 | 含义 | 训练目的 |
|------|------|---------|
| `uniaxial` | 只拉压一个轴，对 x/y/z 分别扫描 | 补各向异性弹性（C11/C22/C33） |
| `biaxial` | 两两组合（xy, xz, yz），同时拉压两个轴 | 补泊松耦合和面内各向异性 |
| `triaxial` | 三轴同时独立扫描 | 全应变张量覆盖，组合数快速增长 |
| `isotropic` | 三轴等比例缩放 | 只补体积-能量曲线（体模量） |

可以自由输入单轴如 `X`，等价于 uniaxial 且只扫 x 方向。
...
开启后程序识别有机分子团簇，应变时将分子作为刚性整体移动，避免分子内键长被错误拉伸。
