...
程序先把候选位点沿指定晶格轴的分数坐标 $s$ 包裹到 $[0,1)$，排序后分箱。第 $b$ 个
分箱中心对应 $t_b\in[0,1]$，其目标成分由起点和终点线性插值：

$$
\mathbf c(t_b)=(1-t_b)\mathbf c_{\mathrm{start}}
+t_b\mathbf c_{\mathrm{end}}.
$$

若该箱有 $N_b$ 个位点，理想计数为 $N_b c_e$。程序先取
$n_e=\lfloor N_b c_e\rfloor$，再把剩余位点按小数余数从大到小补给各元素，保证
$\sum_e n_e=N_b$。所以有限晶胞中的实际比例是最接近目标的整数实现，不会逐箱精确等于
连续比例。
...
**参数设置：** `输入元素`填 `Ni,Co`，`梯度轴`选择 `a`，`分箱数`填 `8`，
`目标元素`填 `Ni,Co`，`每个输入的样本数`填 `3`，并开启`使用随机种子`。
...
`str`，默认 `Ni,Co`。界面将元素、起始比例和结束比例合并在同一张表中；该字段是表格第一列的序列化结果。

### 起始成分（start_composition）

`str`，默认 `Ni:1,Co:0`。对应表格“起始比例”列和 axis 低坐标端。比例不要求归一化，各元素相对比值决定第一层的目标配比。

### 结束成分（end_composition）

`str`，默认 `Ni:0,Co:1`。对应表格“终止比例”列和 axis 高坐标端。扩散偶通常把一个元素从 1 过渡到 0、另一个从 0 过渡到 1。
...
`分箱数`（`bins`）越大，成分变化看起来越细，但每层原子越少、整数取整误差越大。某层有 `N` 个候选原子时，单个元素能够表达的最小成分步长约为 `1/N`。例如每层只有 4 个原子，只能稳定表达 0%、25%、50%、75% 和 100%。如果输入只有 32 个候选原子，设 8 层已经意味着每层约 4 个原子。
...
**层内成分不等于目标小数。** 原子数是整数，每层按最接近的整数计数分配。增大超胞或减少 `分箱数`（`bins`）可以提高每层成分分辨率。

**运行报错或没有输出。** `元素`（`elements`）少于 2 个、起止成分解析失败，或 `目标元素`（`target_elements`）没有匹配到原子。
...
开启 `使用随机种子`（`use_seed`）后，层内元素排布由 `随机种子`（`seed`）、输入结构标识和 sample 序号共同决定。
