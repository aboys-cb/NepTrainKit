...
这张卡采样的是成分单纯形，而不是直接替换原子。对 $K$ 种元素，目标成分满足

$$
x_e\ge x_{\min},\qquad \sum_{e=1}^{K}x_e=1.
$$

`网格`模式在这个约束面上枚举规则点；`Sobol`模式把低差异序列映射到同一约束面，适合在
有限预算下更均匀地覆盖高维成分空间。输出只把目标 $x_e$ 写入 `Comp(...)` 标签，元素占位
仍与输入相同；要得到真实化学结构，必须再接`随机占位`。预算分配会在每个输入结构之间回流
未用完名额，因此“最大输出数”是全流程上限，不保证每个输入得到相同数量。
...
**参数设置：**
- `元素`（`elements`） = `Co,Cr,Ni`
- `组合元数`（`order`） = `2,3` （二元 + 三元组合）
- `采样方法`（`method`） = `Grid`，`成分步长`（`step`） = `0.1`
- `每个输入的最大目标成分数`（`max_outputs`） = `500`
...
- 如果全部集中等摩尔附近但没有边角覆盖，打开 `包含边界成分点`（`include_endpoints`），降低 `元素最小占比`（`min_fraction`）到 0
- 如果输出量不够覆盖所有 order 组合，提高 `每个输入的最大目标成分数`（`max_outputs`），或把 `输出预算分配`（`budget_mode`）换成 `Capacity-weighted` 让高元数组合分到更多名额
...
`int`，默认 50，仅 Sobol 模式生效。它表示每一种元素组合通过 `元素最小占比`（`min_fraction`）约束后的目标有效点数，而不是过滤前的尝试数。operation 会继续补采样直到得到该数量；典型值为 25-200。
...
`int`，默认 500。限制每个输入结构生成的目标配比总数，防止组合爆炸。超出预算时按 `输出预算分配`（`budget_mode`）规则分配到各组合元数；典型值为 250-1500。
...
**输出和输入完全一样。** `元素`（`elements`）少于 2 个元素，或 `组合元数`（`order`）不合法，或 `每个输入的最大目标成分数`（`max_outputs`） <= 0。

**标签成分和原子实际组成不一致。** 正常——Composition Space Sampling 只写目标配比标签，不改原子种类。需要接 `Random Occupancy` 才会真正替换原子。

**组合爆炸。** 5 元 + Grid + step=0.05 可产生上万个点。先用 `每个输入的最大目标成分数`（`max_outputs`）卡住上限，小样本验证后再放量。

**某些 order 没有输出。** 检查 `输出预算分配`（`budget_mode`）分配和 element 数量。只有 3 个元素但 order 含 4 和 5 时，程序自动跳过不可执行的组合。
...
勾选 `使用随机种子`（`use_seed`） + 固定 `随机种子`（`seed`） → 相同输入顺序可复现。Grid 模式的网格点本身是确定性的，`使用随机种子`（`use_seed`）主要影响组合选取顺序和 Sobol 生成。
