...
你在单层 MoS2 上训练了一个 NEP 模型，单层性质很好。然后尝试跑双层 MoS2——层间距离预测偏了 0.5 Å，层间滑移能垒完全不对。
...
**目标：** 用 sin 调制给层加起伏，然后复制 3 层，层间距 6.5 Å
...
- 检查层间是否重叠：最低层的顶部原子和上一层的底部原子距离应 > 2.5 Å
...
`int`，默认 1。选择内置的变换模板。`1` 是自定义模式，配合下面的 `Dz Expr` 写你自己的位移表达式。

#### DZ Expr（dz_expr）

`str`，默认 `sin(x/pi) + sin(y/pi)`。z 方向位移表达式，支持三角函数、指数等数学函数。可用变量：`x`, `y`, `z`, `pi`, `e`。

- `sin(x/pi) + sin(y/pi)`——双向波纹，适合模拟热起伏或应变波
- `0.5 * sin(x/3)`——单向正弦起伏
- `0`——纯堆叠，不给层加任何调制

通过 `Expression Params` 字段可以定义表达式中引用的额外标量参数，格式 `name=value, name=value`。

#### Expression Params（expression_params）

`str`，默认空。你写在位移表达式里引用的自定义参数，用 `name=value, name=value` 的格式填。

### 作用范围

#### Apply Mode（apply_mode）

`int`，默认 0。决定层复制和调制作用在哪些原子上。
...
如果你只想给某一种元素（比如表面 S）加起伏而 Mo 不动，切到按元素模式再填 `Elements`。

#### Elements（elements）

`str`，默认空。仅 `apply_mode=1` 时生效。逗号分隔的元素列表，如 `S,Mo`——注意大小写要和结构里的一致。

#### Z Range（z_range）

`tuple[float, float]`，默认 (-1000000.0, 1000000.0)。仅 `apply_mode=2` 时生效。`[最小值, 最大值]`，按 z 坐标筛选参与调制的原子。

### 层复制和晶胞

#### Layers（layers）

`int`，默认 3。复制出几层。`2` 是双层堆叠，`3` 是三层，以此类推。

#### Distance（distance）

`float`，默认 3.0。层间距离，单位 Å。这个值要根据你的层状材料来设——MoS2 约 6.5 Å，石墨约 3.35 Å。设小了层间会重叠。

#### Wrap（wrap）

`bool`，默认 false。勾选后把复制出来的坐标 wrap 回周期盒里。大多数场景不需要开，保持默认就行。

#### Extend Cell Z（extend_cell_z）

`bool`，默认 true。勾选后自动扩展 z 向盒子高度以容纳所有复制出来的层。建议保持开启，否则新层可能跑到盒子外面去。

#### Extra Vacuum（extra_vacuum）

`float`，默认 0.0。在自动扩展盒子高度的基础上额外加的真空长度。0~5 Å 对大多数场景足够了。

## 推荐预设

### 双层（平整堆叠，2 层，间距 6.5 Å）
```json
{
  "class": "LayerCopyCard",
  "check_state": true,
