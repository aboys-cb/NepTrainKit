...
把一个已有 `spin:R:3` 或矢量磁矩的结构展开成完整、对称且可追踪的局域响应组。单自旋、原子对和 group-pair 都复用 Small-Angle Spin Tilt 的旋转定义；每个目标拥有自己的零角参考帧，输出预算只按完整组截断。Moment magnitude preset 固定方向，只扫描模长。
