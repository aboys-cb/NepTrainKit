...
### Elements（elements）
类型：`str`。默认：`''`。限制哪些元素的磁矩会被随机旋转。

逗号分隔的元素列表，如 `Fe,Co`。只对列出的元素做旋转和模长扰动。留空 = 全部磁性原子参与。

### Max Angle（max_angle）
类型：`float`。默认：`10.0`。设置磁矩随机旋转的最大角度。

最大旋转角，单位度。每次随机采样角度在 [0, max_angle] 内均匀分布。
- 保守：2~5°（验证方向变化确实有帮助）
- 平衡：8~15°（常规有限温度覆盖）
- 探索：20°+（宽温度区间，需重点抽查）

### Num Structures（num_structures）
类型：`int`。默认：`5`。设置每个输入结构生成的随机旋转样本数。

每输入帧输出的旋转版本数。5~10 用于轻量补样，10~30 用于常规覆盖，30+ 建议后接过滤。

### Lift Scalar（lift_scalar）
类型：`bool`。默认：`True`。决定是否把标量磁矩提升为非共线向量。

输入是共线标量磁矩时，是否先沿 `Axis` 抬升为向量再做旋转。输入是标量且需要旋转时必须开启。

### Axis（axis）
类型：`list[float] | tuple[float, float, float]`。默认：`(0.0, 0.0, 1.0)`。设置 cone 随机旋转围绕的参考磁矩方向。

`lift_scalar` 抬升时使用的参考方向，默认 `[0, 0, 1]`。输入已经是向量磁矩时此参数对初始方向无影响。

生效条件：涉及方向、分层、表面或向量初始化的模式都会使用。

### Disturb Magnitude（disturb_magnitude）
类型：`bool`。默认：`True`。决定随机旋转时是否同时扰动磁矩模长。

是否在旋转的同时随机缩放磁矩模长。

### Magnitude Factor（magnitude_factor）
类型：`list[float] | tuple[float, float]`。默认：`(0.95, 1.05)`。设置磁矩模长随机缩放因子的范围。
