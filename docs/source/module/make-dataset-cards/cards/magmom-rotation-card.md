...
若训练集主要是严格 FM/AFM 构型，希望模型同时覆盖方向与模长涨落，可从已有参考磁态出发：

1. 最大偏转角使用默认的 `30°`，覆盖明显偏离参考磁态的方向涨落；
2. 模长范围使用默认的 `0.8–1.2`，同时覆盖约 ±20% 的纵向涨落；
3. 每个输入生成 `5` 个结构并固定 seed；
4. 重训后在独立的磁矩扰动测试集上比较误差，并检查输出磁矩的实际最大偏转和模长范围。
...
`float`，默认 `30.0`，单位度，范围 `0–180`。它是新旧磁矩真实夹角的上界；球冠内按立体角均匀采样，不是每个原子都固定偏转该角度。需要小角度响应路径时使用“可控自旋倾斜”。
...
`tuple[float, float]`，默认 `(0.8,1.2)`。依次为缩放因子下限和上限，对应输入磁矩模长约 ±20% 的随机涨落；两者必须有限、非负且下限不大于上限。
