...
这张卡只负责原型、晶格参数和子晶格占位，不负责扩胞。需要更大结构时，在后面连接 [扩胞（Super Cell）](super-cell-card.md)。
...
选择 `L1₂ / A₃B`，A 填 `Cu`、B 填 `Au`。输出即为固定 3:1 化学计量的 Cu₃Au 基胞；需要更大结构时只需后接“扩胞”，无需再经过占位卡。
...
**为什么 X 不能直接训练？** `X` 不是化学元素，只是未决位点标记。必须先用占位卡替换为真实元素。

**旧配置为什么不再按内部倍率扩胞？** 旧版扩胞字段已经移除。加载时会显示迁移提示，请补上一张“扩胞”卡恢复尺寸。

## 输出合同

- 每个 $a$ 点输出一个未扩展、未弛豫、三维周期基础晶胞。
...
- 旧版 `auto_supercell`、`max_atoms`、`rep` 不再生效；加载时给出提示。
