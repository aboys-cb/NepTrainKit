...
每个有序原型由基胞矩阵、固定分数坐标和每个位点的子晶格标签共同定义。重复数为
$(n_a,n_b,n_c)$ 时，

$$
N'=N_0n_an_bn_c,\qquad
N'_\lambda=N_{\lambda,0}n_an_bn_c,
$$

其中 $N_0$ 是基胞总位点数，$N_{\lambda,0}$ 是子晶格 $\lambda$ 的基胞位点数。
因此 L1₂ 的 A:B=3:1、B2 的 A:B=1:1 会在任何整数扩胞后严格保持。非立方 A3/hcp 和
L1₀ 使用 $c=a(c/a)$。`sublattice` 标签表示位点身份；把 A、B 映射成什么元素只是占位
赋值，不改变原型拓扑。
...
**参数设置：** `原型`选择 `L1₂/A3B`，`晶格常数范围`填 `[3.6,3.6,0.1]`，
`子晶格元素`填 `A:X,B:X`，关闭自动扩胞并把`重复倍数`设为 `[2,2,2]`，
`最大原子数`设为 `32`。
...
需要补体积响应时可扫描一段窄范围；只要一个结构时令起点等于终点。每个扫描点都受 `最大输出数`（`max_outputs`）限制。

### 晶格轴比 c/a（covera）

`float`，默认 1.0。A3/hcp 和 L1₀ 使用该值设置 `晶格轴比 c/a`（`covera`）；立方 A1、A2、L1₂、B2 固定使用 1，并在界面中禁用该输入，避免误以为它会生效。
...
`bool`，默认 true。开启后，在不超过 `最大原子数`（`max_atoms`）的前提下自动选择三轴重复数。
...
如果基胞本身或手动 `重复倍数`（`rep`）超过这个上限，operation 直接抛出 `ValueError`，不会偷偷缩小重复数或返回超预算结构。
...
扫描范围很密时，它是独立于原子数的第二道预算门；达到上限后按 `晶格常数范围`（`a_range`）的确定顺序停止。
...
**手动超胞超过 `最大原子数`（`max_atoms`）。** 卡片会明确失败，不会改变用户给出的重复数。调小 `重复倍数`（`rep`）或提高原子数预算。
