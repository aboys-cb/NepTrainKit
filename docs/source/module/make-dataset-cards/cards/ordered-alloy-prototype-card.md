...
`tuple[float, float, float]`，默认 `(3.75, 3.75, 0.1)`，单位 Å，依次为起点、终点、正步长。默认值对应常见 Cu₃Au L1₂ 基胞；反向端点会交换，每个采样点输出一个基础晶胞。
...
`str`，默认 `A:Cu,B:Au`，可直接生成 Cu₃Au。界面将其拆成 A、B 两个独立输入框；单子晶格原型只显示 A。每项接受一个真实元素符号或 `X`。
