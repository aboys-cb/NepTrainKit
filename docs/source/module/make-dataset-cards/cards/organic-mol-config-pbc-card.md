...
$\Delta\theta$ 是被选可旋转键的子树转角，$\sigma$ 是逐 Cartesian 分量的高斯标准差，
$r_i^{\mathrm{cov}}$ 是元素共价半径。拓扑、键长保护和非键碰撞三道检查共同决定候选是否
接受，公式中的随机抽样本身并不保证化学合理。
...
**参数设置：**
- `每个输入的请求输出数`（`perturb_per_frame`） = `100`
- `扭转角增量`（`torsion_range_deg`） = `[-60, 60]`
- `每个输出旋转的键数`（`max_torsions_per_conf`） = `5`
- `高斯坐标噪声`（`gaussian_sigma`） = `0.03 Å`
...
- 如果成功率低（< 10%），放宽 `最大保留键长系数`（`bond_keep_max_factor`）或增大 `最大重试次数`（`max_retries`）
...
`int`，默认 150。这不是距离 cutoff，而是切换局部模式的**总原子数阈值**。当输入原子数大于该值时，每次旋转的子树会受 `仅旋转局部子树`（`local_subtree`）限制；小体系则旋转完整一侧子树。

#### 仅旋转局部子树（local_subtree）

`int`，默认 40。局部模式中，一条键一侧最多一起旋转的原子数。它只有在输入原子数超过 `局部截断距离`（`local_cutoff`）时生效。

#### 非周期晶胞尺寸（nonpbc_box_size）

`float`，默认 100.0 Å。非周期输出被居中后写入的立方显示 cell 边长。输出 PBC 仍为 false，因此该盒子不会参与碰撞检查，也不会制造周期镜像。

### 键识别和键长约束

#### 成键识别系数（bond_detect_factor）

`float`，默认 1.15。候选键距离必须不超过 `factor * (r_i^cov+r_j^cov)`，同时还要通过 `键级阈值`（`bo_threshold`）。数值越大越容易把相邻分子或配位接触误识别为键。

#### 最小保留键长系数（bond_keep_min_factor）

`float`，默认 0.6。原始检测键允许的最短距离为 `factor * (r_i^cov+r_j^cov)`。它防的是过度压缩，不是断键；设太大会拒绝合理的短键。

#### 最大保留键长系数（bond_keep_max_factor）

`float`，默认 1.15。勾选上限后，原始检测键允许的最长距离系数。

#### 启用最大保留键长（bond_keep_max_enable）

`bool`，默认 false。打开后才启用键长上限检查；关闭时没有隐藏的默认上限。研究非反应构象时建议打开；反应路径或软配位键可能需要关闭。

#### 多重键系数（mult_bond_factor）
...
`int`，默认 0。固定随机种子值。仅 `使用随机种子`（`use_seed`）打开时生效，两个不同 seed 产生两组不同的扭转角和扰动方向。
...
**输出数量明显小于请求值。** 其余候选没有通过保护条件。先检查预览中的检测键和可旋转键是否合理，再收窄 `扭转角增量`（`torsion_range_deg`）、减小高斯噪声，或调整真正导致失败的保护阈值。增大 `最大重试次数`（`max_retries`）只会用更小扰动继续尝试。

**键被拉断了。** `启用最大保留键长`（`bond_keep_max_enable`）没开，或者 `最大保留键长系数`（`bond_keep_max_factor`）太大。开启该约束并设定合理上限。

**非键原子穿插。** `非键原子最小距离系数`（`nonbond_min_factor`）太小。增大到 0.9~1.0。
...
勾选 `使用随机种子`（`use_seed`） + 固定 `随机种子`（`seed`） → 相同输入可复现。构象生成受 seed + 结构稳定 ID 联合控制，不同 seed 产生不同的扭转角和扰动方向。
