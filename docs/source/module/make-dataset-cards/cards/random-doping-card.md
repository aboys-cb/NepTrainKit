...
`percent` 必须落在 0~100；百分比换算成离散原子数时向下取整，因此小超胞的低百分比可能得到 0 个替换。明确设置 0% 会保留原结构，不会被强制改成 1 个掺杂原子。固定数量或随机数量一旦超过当前候选位点数会报错，不会静默截断。填写 group 后，如果输入没有 `atoms.arrays["group"]`、标签未命中或目标元素未命中，也会明确报错。
...
**提示请求数量超过候选位点。** 卡片不会替你把 10 个改成“最多能换的 4 个”。请减少 `count`、扩大超胞，或检查前一条规则是否已经消耗了相同 target/group 的候选位点。

**多条规则之间的交互。** 规则按顺序执行。如果两条规则操作同一个 target 元素，第二条只会看到第一条替换后仍属于 target 的位点。数量冲突会明确报错。
