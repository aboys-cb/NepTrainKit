...
程序在输入晶胞内依次随机放置原子。候选分数坐标
$\mathbf s\sim U([0,1)^3)$，Cartesian 坐标为
$\mathbf r=\mathbf s^\mathsf T\mathbf C$。对新原子 $i$，只有满足

$$
d_{ij}^{\mathrm{MIC}}\ge d_{\min}(Z_i,Z_j)
\qquad \forall j<i
$$

才接受该位置。元素对专用阈值优先于全局最小距离；所有距离单位为 Å。程序优先放置约束更大
的元素，以降低后期无位置可放的概率，但它仍是带拒绝的随机顺序算法，不保证在高密度条件下
一定找到解。达到每原子最大尝试次数后，严格模式报错，非严格模式跳过该输出样本。
...
参数设置：`structures=20`，`composition=""`，`min_distance=1.5`，`pair_min_distances="Fe-O:1.8,O-O:1.2"`，`strict_mode=True`，打开 `使用随机种子`（`use_seed`）。
...
`str`，默认空。留空 = 沿用输入结构的元素种类和原子计数。界面使用“元素 / 原子数”表格；序列化仍写成 `Fe:32,O:64`，以兼容旧工作流。
...
`str`，默认空。界面高级区域使用“元素对 / 最小距离（Å）”表格；序列化仍写成 `Fe-O:1.8,O-O:1.2`。表中规则覆盖全局距离。

元素半径差异大时你会想用这个——O-O 通常比 Fe-O 可以更近一些，同时 Fe-O 可能需要比全局 `最小距离`（`min_distance`）更宽松。写在 pair 里的距离覆盖全局值，没写的元素对仍然用 `最小距离`（`min_distance`）。

#### 每个原子最大尝试次数（max_attempts_per_atom）

`int`，默认 500。每个原子最多随机尝试多少次放置位置，超过就认为放不下。

增大它只增加搜索时间，不改变物理可行性。如果你发现它一直失败（几乎每次都触达 500 次上限），不是该调大它——是你的 `最小距离`（`min_distance`）设大了，或者 cell 相对原子数太小。先调松距离或扩胞。

#### 严格模式（strict_mode）

`bool`，默认 true。打开：只要有一个原子放不下，整个样本就报错，保证输出数量 = 设定值。关闭：放不下的样本被跳过，最终输出可能比你设的 `结构数`（`structures`）少。
...
`int`，默认 0。仅 `使用随机种子`（`use_seed`）打开时生效。
...
开启 `使用随机种子`（`use_seed`）后，随机 packing 由 `随机种子`（`seed`）、输入结构稳定 ID 和 sample 序号共同决定。相同输入、相同参数、相同 seed 会生成相同坐标。
