...
对每个组合调用周期表面构造，把 $(hkl)$ 平面作为表面法向，沿法向堆叠 $L$ 层；
$v>0$ 时在法向加入对应 Å 数的真空，$v=0$ 时不额外加真空。输出数的理论上限是各扫描
点数的乘积减去 $(0,0,0)$ 组合。失败的不可构造晶面会记录并跳过，因此实际输出可能更少。
...
**参数设置：**
- `Miller 指数 h 范围`（`h_range`） = `[0, 1, 1]`，`Miller 指数 k 范围`（`k_range`） = `[0, 1, 1]`，`Miller 指数 l 范围`（`l_range`） = `[1, 3, 1]`
- `Layer Range` = `[3, 6, 1]`
- `真空层范围`（`vacuum_range`） = `[10, 10, 1]`
...
- 如果表面能预测仍不准，扩大 `Miller 指数 l 范围`（`l_range`）到 `[1, 5, 1]` 覆盖更多高指数面
- 如果 slab 太薄表面效应过强，增大 `层数范围`（`layer_range`）的下限
