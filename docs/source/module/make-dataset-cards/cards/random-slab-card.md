...
类型：`Sequence[int]`。默认：`(0, 1, 1)`。设置 slab Miller h 指数范围。

物理直觉：根据这张卡要补的训练集缺口设置；调整后重点检查输出数量、几何合理性和 `Config_type` 标签是否符合预期。

### K Range（k_range）

类型：`Sequence[int]`。默认：`(0, 1, 1)`。设置 slab Miller k 指数范围。

物理直觉：根据这张卡要补的训练集缺口设置；调整后重点检查输出数量、几何合理性和 `Config_type` 标签是否符合预期。

### L Range（l_range）

类型：`Sequence[int]`。默认：`(1, 3, 1)`。设置 slab Miller l 指数范围。

物理直觉：根据这张卡要补的训练集缺口设置；调整后重点检查输出数量、几何合理性和 `Config_type` 标签是否符合预期。

### Layer Range（layer_range）

类型：`Sequence[int]`。默认：`(3, 6, 1)`。设置 slab 层数范围。

物理直觉：根据这张卡要补的训练集缺口设置；调整后重点检查输出数量、几何合理性和 `Config_type` 标签是否符合预期。

### Vacuum Range（vacuum_range）

类型：`Sequence[int]`。默认：`(10, 10, 1)`。设置 slab 真空层厚度范围。
