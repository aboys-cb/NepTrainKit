...
每条规则先用元素和可选 group 得到候选集合 $\mathcal I_r$。`固定数量`从中无放回抽取
指定的 $n_r$ 个位点；`随机数量`先在整数区间
$[n_{r,\min},n_{r,\max}]$ 抽取 $n_r$，再抽位点。单条规则理论上有

$$
\binom{|\mathcal I_r|}{n_r}
$$

种删除组合。多条规则依次应用并检查重复结果；若请求数量超过候选位点数会直接报错，而不是
暗中降低空位数。固定种子会结合输入结构派生随机路径，使同一结构和参数可复现。
...
**参数设置：**
- `规则`（`rules`）：`[{"element":"O","count_mode":"random","count":[1,3],"group":["surface"]}]`
- `Maximum Outputs per Input`：`20`
- `Use Seed`：勾选，`随机种子`（`seed`）：`[42]`
...
勾选 `使用随机种子`（`use_seed`）并固定 `随机种子`（`seed`）后，随机序列由 seed 与单帧结构内容共同决定：同一结构可复现，不同结构不会机械复用同一组原子编号，数据集重新排序也不改变单帧结果。
