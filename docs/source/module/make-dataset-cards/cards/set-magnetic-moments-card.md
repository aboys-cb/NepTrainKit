...
**`Source`**（source）：决定磁矩从哪里来。

| 选项 | 含义 |
|------|------|
| `Map/default magnitude` | 用 `magmom_map` / `default_moment` 生成。输入不需要已有磁矩 |
| `Existing initial magmoms` | 保留输入已有的磁矩，只做格式转换。输入需要已有 `initial_magmoms` |
| `Constant magnitude` | 所有目标元素统一用一个常数幅值，适合快速扫参考值 |

### 输出格式

**`Format`**（format）：`Collinear (scalar)` 或 `Non-collinear (vector)`。后续要做旋转、螺旋、canting 时优先选向量格式。

### 参考轴

**`Axis`**（axis）：`[x, y, z]`。向量输出的参考方向、标量抬升的方向。

### 磁矩幅值

**`Magmom Map`**（magmom_map）：元素到磁矩的映射。格式 `Fe:2.2,Ni:0.6`，也可用 JSON 向量如 `{"Cr":[0,0,1.0]}`。

**`Use Element Dirs`**（use_element_dirs）：勾选后，`magmom_map` 中提供的向量方向会被保留，而不是统一投到 `Axis`。仅向量格式下有意义。

**`Default Moment`**（default_moment）：`magmom_map` 未命中元素的默认幅值。设 0 则不给未配置元素写磁矩。

**`Constant Moment`**（constant_moment）：`Source = Constant magnitude` 时所有目标元素的统一幅值。

### 格式转换

**`Lift Scalar`**（lift_scalar）：输入是标量且目标格式是向量时，是否沿 `Axis` 抬升。通常保持开启。
