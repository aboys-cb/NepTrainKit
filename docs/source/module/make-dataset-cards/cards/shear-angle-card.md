...
- 控件解释 (Widget): 由 alpha/beta/gamma 角度范围和有机识别控件组合生成的内部参数字典。
- 类型/范围 (Type/Range): dict
- 默认值 (Default): `{"alpha_range": [-2.0, 2.0, 1.0], "beta_range": [-2.0, 2.0, 1.0], "gamma_range": [-2.0, 2.0, 1.0], "identify_organic": false}`
- 含义 (Meaning): UI-independent 参数快照，供 core operation、测试和未来批处理入口复用。
- 对输出规模/物理性的影响: 本字段本身不新增物理行为；其内容与下面的 legacy 字段保持同一组晶格角扰动参数。
