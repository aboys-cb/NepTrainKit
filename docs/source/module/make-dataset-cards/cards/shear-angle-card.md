...
**`alpha_range`**（list[3]，默认 `[-2, 2, 1]` (°)）：Alpha 角增量扫描区间 `[min, max, step]`，单位度。表示相对原始角度的偏移量。不扫的通道设为 `[0, 0, 1]` 保证至少生成一个点。

**`beta_range`**（list[3]，默认 `[-2, 2, 1]` (°)）：Beta 角增量扫描区间，同上。

**`gamma_range`**（list[3]，默认 `[-2, 2, 1]` (°)）：Gamma 角增量扫描区间，同上。

**`identify_organic`**（bool，默认 false）：有机团簇识别。分子晶体必须开启。
