...
### HKL（hkl）
类型：`Sequence[int]`。默认：`(1, 1, 1)`。设置要切开的晶面 Miller 指数。

Miller 指数，定义滑移面。

- 低指数（111, 110, 100）：最常见的滑移面，先从这里开始
- 中指数（222, 311）：对应次密排面
- 高指数（333+）：通常对应更高阶滑移系统，建议先用低指数验证
...
- 保守：`[0.0, 0.3, 0.1]`，4 个点
- 平衡：`[0.0, 0.6, 0.1]`，7 个点
- 探索：`[0.0, 1.0, 0.05]`，21 个点

### Layers（layers）
类型：`int`。默认：`1`。设置层复制或层错相关的层数。

从哪一层开始做相对位移。整数索引。

- 单层滑移（1）：默认值，适合大多数场景
- 多层滑移（2-5）：适合研究厚 slab 中间层的错排
