...
这张卡沿晶格矢量 $\mathbf a$、$\mathbf b$、$\mathbf c$ 整数次复制完整输入晶胞。它不会施加应变、改变键长或自动弛豫结构。
...
例如输入有 2 个原子、长度为 $(5,6,7)$ Å，上限为 100。程序选择 $(5,5,2)$：输出恰好 100 个原子，长度为 $(25,30,14)$ Å。它不会选择同样有 100 个原子但极细长的 $(50,1,1)$。
...
**为什么原子预算没有恰好用满？** 只有预算允许合适的整数倍率乘积时才能恰好用满；程序不会删原子或改变组成凑数。
