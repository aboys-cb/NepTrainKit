...
类型：`int`。默认：`1`。设置按百分比删除原子的范围。

count 模式下的空位数量。范围为 1-8。

#### Use Num（use_num）
类型：`bool`。默认：`True`。决定空位数量使用绝对计数还是浓度范围。

物理直觉：打开后按绝对空位数删除，适合固定缺陷数；关闭后按浓度范围删除，适合不同超胞尺寸下保持缺陷比例。

#### Concentration Condition（concentration_condition）
类型：`float`。默认：`0.0`。设置空位浓度范围。

fraction 模式下的空位比例。0.02 表示约 2% 的原子被删。推荐 0.005-0.08。

#### Count Mode（count_mode）
类型：`str`。默认：`'fixed'`。选择空位计数生成方式。

`fixed` 精确删除上面指定的数量；`random` 在 1 到指定数量之间随机。新建卡片默认 `fixed`，只有显式选择 `Random up to value` 时才随机空位数量。
