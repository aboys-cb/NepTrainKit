...
**和 `Targeted Vacancy` 的区别：**

- `Global Random Vacancy`：所有元素都可被删除，适合快速覆盖整体空位数量或比例。
- `Targeted Vacancy`：按元素和可选 `group` 规则选择候选位点，适合子晶格或特定元素空位。
...
- 需要按元素或子晶格删位时，改用 `Targeted Vacancy`，不要把两张卡串联来模拟一条规则。

## 常见问题

**为什么多元素结构的组分会变化？**

所有原子都是候选位点。若需要只删除 O、某种金属或某个 `group`，使用 `Targeted Vacancy`。
