...
模态系数采样分布。`0` = 高斯（均值为 0，更接近热振动统计）；`1` = 均匀。高斯模式下大幅位移概率更低，通常更物理。

### Amplitude（amplitude）
类型：`float`。默认：`0.05`。设置振动模态位移幅值。

模态叠加的总幅值系数。实际位移 = `amplitude * sum(coefficient * mode_vector)`。较小值（0.01~0.03）适合微扰验证；0.05~0.08 适合常规补充；0.1+ 需要后筛检查。注意：此幅值适用于模态归一化后的矢量，如果模态本身归一化方式不同（mass-weighted vs. 不归一化），实际原子位移也不同。

### Modes Per Sample（modes_per_sample）
类型：`int`。默认：`2`。设置每个样本叠加的振动模态数量。

每个输出结构叠加的模态数量。1~2 适合单模态方向验证；3~4 适合多模态混合覆盖；5+ 模态组合数指数增长，单个模态贡献稀释。

### Min Frequency（min_frequency）
类型：`float`。默认：`10.0`。设置参与采样的最低频率。

最小频率阈值。仅当 `exclude_near_zero=true` 时生效。频率低于此值的模态被排除。设 0 则不过滤。典型值：排除 < 0.1 THz 的平移/旋转伪模，设 0.1~1.0；排除软模，视体系设 1~10。

### Max Num（max_num）
类型：`int`。默认：`32`。设置扰动或采样结构的输出数量。

每输入帧生成的扰动样本数。20~40 常规覆盖；50~100 高密度覆盖。

### Scale BY Frequency（scale_by_frequency）
类型：`bool`。默认：`True`。决定是否按频率缩放模态位移。

按频率缩放模态贡献。开启后每个模态系数除以 sqrt(frequency)，高频模态贡献自动衰减。这是物理上更合理的设置——在同样的位移幅度下，高频模态需要更多能量。研究大位移非谐效应时可关闭。

### Exclude Near Zero（exclude_near_zero）
类型：`bool`。默认：`True`。决定是否排除近零频模态。

排除近零频模态。开启后用 `min_frequency` 做阈值过滤。建议保持开启，否则平移/旋转伪模可能产生异常大位移。
