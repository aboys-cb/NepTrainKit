...
- 用一串无关的“不做什么”定义功能，例如在原子扰动页罗列“不做弛豫、不做能量筛选”。如果用户从卡片名称和参数中本来就不会期待某项能力，不要专门否定它；直接说明真实效果或需要处理的具体风险。
...
每个原子的最大位移向量长度。这是绝对长度（单位 Å），不是每个 Cartesian 分量的上限，也不是百分比。室温下典型固体原子振动幅度约 0.05~0.1 Å，设 0.15~0.3 Å 通常合适。含 H 等轻元素时考虑开启 `use_element_scaling` 单独调大 H 的幅度。
```
...
这个值不是 Cartesian 分量上限，也不是应变百分比——就是一个原子最多被挪多远。室温下原子振动幅度约 0.05~0.1 Å，设 0.15~0.3 Å 通常够用。如果体系含 H 等轻元素，H 的振幅天然更大，建议同时开启 `use_element_scaling` 单独给 H 调大。设到 0.3~0.5 Å 以上时，跑完务必抽查最近邻键长——这个量级已经可能拉断键了。

生效条件：开启 `use_element_scaling` 后，对在 `element_scalings` 中列出的元素，这个值会被覆盖。
```

变化点：
- 第一行不再是独立的 "类型：X。默认：Y。"，而是压缩成 `类型，默认值。描述。`
- "你"出现了——用户在读一个人写的东西
- "如果...建议..." 替代了 "考虑开启"
- "务必" 替代 "需要"——人的语气
...
- 无关能力免责声明。只有当卡片名称或参数容易让用户产生合理误解，或者遗漏该边界会直接导致错误结果时，才说明“不会/不支持”；否则改写为实际效果、风险或下一步动作。
