...
**参数设置：** rules(target=Si,dopants=Ge:1.0,use=atomic_percent,percent=[3,8]), doping_type=Exact, max_atoms_condition=[20]
**输出：** 20 个掺杂结构
...
两种风格都可以，但必须覆盖每个参数的类型、默认值和场景化建议。

### 风格 A：紧凑段落式（参数 ≤ 8 个的卡片推荐）

按功能分组，每段用 `**名称**（key）：` 开头，直接说明类型、默认值、含义、使用场景。
...
| 选项 | 含义 | 训练目的 |
|------|------|---------|
| uniaxial | 只拉压一个轴 | 补各向异性弹性（C11/C22/C33）|
| isotropic | 三轴等比例缩放 | 只补体积-能量曲线 |
```

### 风格 B：逐参数 block 式（参数 ≥ 10 个的卡片推荐）

沿用 `### \`key\` (Label)` 格式，但字段从旧格式缩减为以下核心字段：

- `类型/范围 (Type/Range)`
- `默认值 (Default)`
- `含义 (Meaning)`
- `使用建议`（场景 + 量级，替代旧格式的"物理直觉/典型值"+"怎么判断该开还是该关"）
- `参数联动 / 生效条件`（仅在参数有条件生效时写）
